...
const MisCitas = () => {
  ...
  const aceptarCita = async (idCita) => {
    try {
      ...
      const response = await fetch('http://localhost:3000/citas/actualizar-estado', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const citasFiltradas = filtroEstado === 'Todas' 
    ? citas 
    : citas.filter(cita => cita.estado && cita.estado.toLowerCase() === filtroEstado.toLowerCase());
  ...
  return (
    <div className="mis-citas-container">
      <div className="citas-header">
        <h1>Mis Citas</h1>
        <Link to="/profile" className="btn-volver">Volver al perfil</Link>
      </div>
      ...
      {citas.length > 0 ? (
        <div className="citas-lista">
          {citasFiltradas.map(cita => (
            <div key={cita.id_cita} className={`cita-card estado-${cita.estado ? cita.estado.toLowerCase() : 'pendiente'}`}>
              ...
              <div className="cita-detalles">
                <p><strong>Servicio:</strong> Entrenamiento personal</p>
                {tipoUsuario === 'cliente' ? (
                ...
                <p className={`cita-estado estado-${cita.estado ? cita.estado.toLowerCase() : 'pendiente'}`}>
        ...
        <div className="no-citas">
          ...
          <Link to="/citas" className="btn-agendar">Agendar una cita</Link>
        </div>
      )}
      
      <div className="citas-footer">
        <Link to="/gimnasios" className="btn-agendar-nueva">Agendar nueva cita</Link>
      </div>
    </div>
  );
};
